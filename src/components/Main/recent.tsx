    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://yt-api.sujitdwivediii.workers.dev/recently-added/"
        );
    ...
    <div className="content_grid w-full h-[calc(100vh_-_(2_*_var(--header-height)))] max-w-[calc(var(--ytd-rich-grid-content-max-width)_+_var(--ytd-rich-grid-item-margin))] pt-[var(--grid-padding)] pl-[16px] pr-[10px] flex flex-wrap overflow-auto xs:p-0 xs:m-0 xs:max-w-none">
      {loading
            ...
            >
              ...
              <div className="content_inside flex ">
                <div className="w-full">
                  <div className="content_name rounded-full text-transparent text-base font-medium overflow-hidden block max-h-16.4 clamp-[2] box-[vertical] text-ellipsis whitespace-normal my-[12px]  mb-1 xs:mb-[2px] xs:mx-3 bg-[var(--hover-color)] h-4 w-4/4">Loading</div>
                  <div className="content_info flex text-Secondary text-sm flex-nowrap">
                    <div className="content_ep text-transparent rounded-full bg-[var(--hover-color)] h-3 w-3/4 xs:mx-3">Loading</div>
        ...
        : data.map((item, index) => (
            <div
              key={index}
              className="content_div flex flex-col relative ml-[calc(var(--ytd-rich-grid-item-margin)_/_2)] mr-[calc(var(--ytd-rich-grid-item-margin)_/_2)] mb-10 w-[calc(100%_/_var(--ytd-rich-grid-items-per-row)_-_var(--ytd-rich-grid-item-margin)_-_0.01px)] xs:rounded-none xs:p-0 xs:m-0 xs:mb-10 xs:w-full xs:max-w-none"
            >
              <div
